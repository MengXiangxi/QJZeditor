...
def sepaFile(filename):
    ...
    while not done:
        ...
        if linebuff != '':
            if len(linebuff)>8:
                if linebuff[:8].decode('gbk') == u'发信人: ': # 判断在bgk下有没有标志字
                    utfseed = 0
                    break
                else:
                    utfseed = 1
            ...
            if linebuff[:8].decode('gbk') == u'发信人: ':
                ...
                while (linebuff[:2] != '--'): # 用“--”来表示一个分区的结束
                    ...
                    if (linenum == 1): # 第二行的位置找分区名称
                        groupchar = linebuff[8] # 分区名称存起来
                        if len(linebuff)>15 : # 如果文件名有"kong"/"void"，则一定超长
                            if linebuff[15] == 'k':
                                kong = 1
                            elif linebuff[15] == 'v':
                                kong = 1
                ...
                GrpFile = open(os.path.join(os.getcwd(),'QJZ@'+filename,groupchar+'@'+filename+'.grp'),'w')
...
if termmode == '?': # termmode为“?”，说明未经初始化
    ...
    if initchiefname == '0':
        ...
        while (editorcolordict.get(initchiefname, '0') == '0'): # 对着编辑字典查一下
            print u'没有找到这个主编，请核对更改或在版上反映该问题。'
            print u'请输入更改后的主编ID：'
            initchiefname = raw_input('')
            if initchiefname == '0':
                break
        chiefedname = initchiefname 
...
headerfile.write(unicode(colorcodewday)) # 色彩代码
...
while(changeeditor[0] == 'y' or changeeditor[0] == 'Y'):
    ...
    # ID纠错
    while(editorcolordict.get(changeeditorvalue, '0') == '0'):
        print u'没有找到这个采编，请核对拼写或更新Editor.ans文件。'
        print u'是否修改拼写？（y/N）'
        changeeditorcont = raw_input('')
        changeeditorcont += '_'
        if (changeeditorcont[0] == 'y' or changeeditorcont[0] == 'Y'):
            print u'请输入更改后采编的ID：'
            changeeditorvalue = raw_input('')
        else:
            changeeditorvalue = '0'
    editordict[changeeditorkey] = changeeditorvalue # 实现采编的更改
...
# 纠错
while (editorcolordict.get(chiefedname, '0') == '0'):
    print u'没有找到这个主编，请核对拼写或更新Editor.ans文件。'
    print u'请重新输入主编ID：（实在不行kill掉进程去查错吧。）'
    chiefedname = raw_input('')
...
# 纠错
while (editorcolordict.get(proofname, '0') == '0'):
    print u'没有找到这个校对，请核对拼写或更新Editor.ans文件。'
    print u'请重新输入校对ID：（实在不行kill掉进程去查错吧。）'
    proofname = raw_input('')
...
for i in footerfile:
    QJZfinalfile.write(i)
QJZfinalfile.write(u'\u001B[37mPowered by QJZEditor '+ver+u'\u001B[m')
footerfile.close()
QJZfinalfile.close()
