...
while(changeeditor[0] == 'y' or changeeditor[0] == 'Y'):
    ...
    if changeeditorkey[0] == '+':
        ...
        while(converteditor(changeeditorvalue) == '0'):
             ...
             print( u'请输入更改后采编的ID：')
             changeeditorvalue = raw_input('')
        addeditor.append(converteditor(changeeditorvalue))
        print( u'已加入采编ID：'+ addeditor[-1])
        print( u'是否继续更改其他分区的主编？（y/N）')
        changeeditor = raw_input('')
        changeeditor += '_'
        if changeeditor[0] == 'y' or changeeditor[0] == 'Y':
            continue
        else:
            break
